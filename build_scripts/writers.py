...
def create_module_directory(config: Config):
    ...
    if isdir(
        f"{normalized_repository_path()}/{config.package_name}/{config.module_name}"
    ):
...
def retrieve_argument_types(method: Method) -> Set[type]:
    ...
    for arg in method.arguments:
        types.add(arg.type) if not isinstance(arg.type, tuple) else [
            types.add(t) for t in arg.type
        ]
    return types
...
def retrieve_types_from_service_resource(
...
def retrieve_types_from_collection(
    service_resource: Union[ServiceResource, Resource]
) -> Set[type]:
    types = set()
...
def write_client(client: Client, config: Config):
    ...
    if not isdir(
        f"{normalized_repository_path()}/{config.package_name}/{config.module_name}/{normalized_module_name}"
    ):
        mkdir(
            f"{normalized_repository_path()}/{config.package_name}/{config.module_name}/{normalized_module_name}"
        )
...
'''
    ...
    {def_} {method.name}(
        ...
        {f"# type: (...) -> {normalize_type_name(method.return_type)}"}
{doc}        {method_body}
"""
    )
...
def write_service_resource(
    ...
    with open(file_path, "w") as file_object:
        ...
        if Boto3ServiceResource not in retrieve_types_from_service_resource(
            service_resource
        ):
...
def write_services(session: Session, config: Config):
    ...
    write_setup(config)
    copyfile(
        config.license, f"{normalized_repository_path()}/{config.package_name}/LICENSE"
    )
    copyfile(
        config.readme, f"{normalized_repository_path()}/{config.package_name}/README.md"
    )
...
def write_init_files(init_files: Dict[str, List], config: Config):
    for module, imports in init_files.items():
        if imports:
            file_path = f"{normalized_repository_path()}/{config.package_name}/{config.module_name}/" f"{normalize_module_name(module)}/__init__.py"
            with open(file_path, "w") as file_object:
                file_object.write(
...
def write_setup(config: Config):
    with open(
        f"{normalized_repository_path()}/{config.package_name}/setup.py", "w"
    ) as file_object:
        file_object.write(
            f"""
